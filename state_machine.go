...
func (stateMachine *StateMachine) Update(sEntries []dstatemachine.Entry) (rentries []dstatemachine.Entry, err error) {
	for sIndex, sEntry := range sEntries {
		...
		// 解码
		if err = update.Unmarshal(sEntry.Cmd); err != nil {
			result.Index = -1
			result.Error = err.Error()
			sEntries[sIndex].Result.Data, _ = result.Marshal()
		...
		err = stateMachine.db.Update(func(txn *badger.Txn) (err error) {
			result.Data = make([]*pb.Items, len(update.Entrys))

			for index, entry := range update.Entrys {
				if result.Data[index], err = stateMachine.updateEntry(txn, entry, eventBatch, update.Response); err != nil {
			...
			// 编码 result 错误
			if sEntries[sIndex].Result.Data, err = result.Marshal(); err != nil {
				return
			}
			return
		})
...
func (stateMachine *StateMachine) Lookup(e interface{}) (out interface{}, err error) {
	...
	// 遍历查询
	result.Data = make([]*pb.Items, len(lookup.Entrys))
	err = stateMachine.db.View(func(txn *badger.Txn) (err error) {
		for index, entry := range lookup.Entrys {
			if result.Data[index], err = stateMachine.lookupEntry(txn, entry, lookup.Response); err != nil {
				result.Index = int32(index)
				return
			}
		}
...
// 解析 data
func (stateMachine *StateMachine) recoverItem(r io.Reader, size uint32) (item *pb.Item, err error) {
	b := make([]byte, size)
...
func (stateMachine *StateMachine) lookupEntry(txn *badger.Txn, entry *pb.LookupEntry, resultField *pb.ResultField) (pbItems *pb.Items, err error) {
	// 查询
	var pbItem *pb.Item
	err = stateMachine.query(txn, entry.Query, resultField, func(item *badger.Item) (err error) {
		if pbItem, err = stateMachine.toPBItem(item, resultField); err != nil {
			return
		}
		pbItems.Items = append(pbItems.Items, pbItem)
		return
	})
	if err != nil {
		return
...
func (stateMachine *StateMachine) updateEntry(txn *badger.Txn, entry *pb.UpdateEntry, eventBatch *StateMachineEventBatch, resultField *pb.ResultField) (pbItems *pb.Items, err error) {
	// 不创建 更新
	...
	if emptyCondition(entry.Query.Key) && emptyCondition(entry.Query.Value) && entry.Query.Limit == 0 {
		...
		// 修改单个
		if pbItem, err = stateMachine.updateEntryOne(txn, entry, eventBatch, item, resultField); err != nil {
			return
		}
...
func (stateMachine *StateMachine) query(txn *badger.Txn, pbQuery *pb.Query, resultField *pb.ResultField, cb func(item *badger.Item) (err error)) (err error) {
	...
	it := txn.NewIterator(badger.IteratorOptions{
		...
		PrefetchValues: resultField.Value,
...
func (stateMachine *StateMachine) updateEntryOne(txn *badger.Txn, entry *pb.UpdateEntry, eventBatch *StateMachineEventBatch, item *badger.Item, resultField *pb.ResultField) (pbItem *pb.Item, err error) {

	// 动作 匹配
	switch entry.Action {
	case pb.UpdateAction_CREATE:
		// 创建  存在 跳过
		if item != nil {
			return
		}
	case pb.UpdateAction_UPDATE:
		// 更新 不存在 返回
		if item == nil {
			return
		}
	case pb.UpdateAction_DELETE:
		// 删除 不存在 返回
		if item != nil {
			return
		}
	...
	// 删除
	if entry.Expire == 1 || entry.Action == pb.UpdateAction_DELETE {
		// 不存在 直接返回
		if item == nil {
			return
		}
...
func (stateMachine *StateMachine) match(val []byte, condition *pb.Condition) (ok bool) {
	ok = true
...
func (stateMachine *StateMachine) valueMatch(item *badger.Item, condition *pb.Condition) (ok bool, err error) {
	// 匹配成功
	ok = true
...
func (stateMachine *StateMachine) toPBItem(item *badger.Item, resultField *pb.ResultField) (pbItem *pb.Item, err error) {
	if resultField.Key {
...
func (stateMachine *StateMachine) toPBEventItem(item *badger.Item) (pbItem *pb.Item, err error) {
	if pbItem.Key, err = stateMachine.ParseKey(item.KeyCopy(nil)); err != nil {
		return
	}
	if pbItem.Value, err = item.ValueCopy(nil); err != nil {
...
func emptyCondition(condition *pb.Condition) bool {
	if condition == nil {
		return true
	}
	if len(condition.Prefix) != 0 {
